...
public class ParallaxScript : MonoBehaviour
{
 Transform cam; // Camera reference (of its transform)
	...
	void Update () {
		if (distanceX != 0f) {
			float parallaxX = (previousCamPos.x - cam.position.x) * distanceX * Time.fixedDeltaTime * 12.5f;
			Vector3 backgroundTargetPosX = new Vector3(transform.position.x + parallaxX, 
			                                          transform.position.y, 
			                                          transform.position.z);
			
			// Lerp to fade between positions
			transform.position = Vector3.Lerp(transform.position, backgroundTargetPosX, smoothingX * Time.fixedDeltaTime);
		}
		...
		if (distanceY != 0f) {
			...
			transform.position = Vector3.Lerp(transform.position, backgroundTargetPosY, smoothingY * Time.fixedDeltaTime);
		}
